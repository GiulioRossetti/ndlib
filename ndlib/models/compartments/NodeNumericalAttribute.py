...
class NodeNumericalAttribute(Compartiment):
    def __init__(self, attribute, value=None, op=None, probability=1, **kwargs):
        ...
        if self.operator is not None and self.operator in self.__available_operators:
            if self.operator == "IN":
                if (
                    not isinstance(self.attribute_range, list)
                    or self.attribute_range[1] < self.attribute_range[0]
                ):
                ...
                if not isinstance(self.attribute_range, int):
                    if not isinstance(self.attribute_range, float):
                        raise ValueError(
                            "A numeric value is required to test the selected condition"
                        )
        else:
    ...
    def execute(self, node, graph, status, status_map, *args, **kwargs):
        ...
        if self.operator == "IN":
            ...
            ) and self.__available_operators[self.operator][1](
                val, self.attribute_range[1]
            )
        else:
            condition = self.__available_operators[self.operator](
                val, self.attribute_range
            )
