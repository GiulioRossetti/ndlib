...
class NodeNumericalVariable(Compartiment):
    ...
    def validate(self):
        if not isinstance(self.variable, str):
            raise ValueError(
                "The variable should be a string pointing to an attribute or status"
            )
        ...
        if self.operator is not None and self.operator in self.__available_operators:
            if self.operator == "IN":
                if (
                    ...
                    or not (
                        isinstance(self.value[0], int)
                        or isinstance(self.value[0], float)
                    )
                    or not (
                        isinstance(self.value[1], int)
                        or isinstance(self.value[1], float)
                    )
                ...
                if self.value_type is None:
                    if not isinstance(self.value, int):
                        if not isinstance(self.value, float):
                            raise ValueError(
                                "When no value type is defined, the value should be numerical"
                            )
                else:
                    if not isinstance(self.value, str):
                        raise ValueError(
                            "The value should be a string pointing to an attribute or status when value type is set"
                        )
        else:
            raise ValueError("The operator provided '%s' is not valid" % operator)

    def execute(
        self, node, graph, status, status_map, attributes=None, *args, **kwargs
    ):
        ...
        if self.operator == "IN":
            condition = self.__available_operators[self.operator][0](
                val, testVal[0]
            ) and self.__available_operators[self.operator][1](val, testVal[1])
        else:
