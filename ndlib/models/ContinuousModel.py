...
class ContinuousModel(DiffusionModel):
    def __init__(
        ...
        if iteration_schemes:
            ...
            self.iteration_schemes.append(
                {"name": "", "function": lambda graph, status: graph.nodes}
            )
        else:
            self.iteration_schemes = [
                {"name": "", "function": lambda graph, status: graph.nodes}
            ]
        ...
        if save_file:
            if isinstance(save_file, str):
                ...
                raise ValueError(
                    "save_file should be a string indicating path/and/filename"
                )
        else:
            self.save_file = None
    ...
    def configure_visualization(self, visualization_configuration):
        ...
        if visualization_configuration:
            ...
            if "variable_limits" not in vis_keys:
                self.visualization_configuration["variable_limits"] = {
                    key: [-1, 1] for key in list(self.available_statuses.keys())
                }
            else:
                for key in list(self.available_statuses.keys()):
                    if key not in list(
                        self.visualization_configuration["variable_limits"].keys()
                    ):
                        self.visualization_configuration["variable_limits"][key] = [
                            -1,
                            1,
                        ]
            if "animation_interval" not in vis_keys:
                ...
                if not isinstance(
                    self.visualization_configuration["animation_interval"], int
                ):
                    raise ValueError("animation interval must be an integer")
            print("Done configuring the visualization")
        else:
            raise Exception(
                "Provide a visualization configuration when using this function"
            )
    ...
    def configure_layout(self, vis_keys):
        if "layout" in vis_keys:
            if self.visualization_configuration["layout"] == "fr":
                ...
                positions = {
                    node: {"pos": location} for node, location in enumerate(layout)
                }
            else:
                if "layout_params" in vis_keys:
                    pos = self.visualization_configuration["layout"](
                        self.graph.graph,
                        **self.visualization_configuration["layout_params"]
                    )
                else:
    ...
    def validate_plot_config(self, visualization_configuration, vis_keys):
        ...
        if "plot_title" in self.visualization_configuration.keys():
            ...
            self.visualization_configuration["plot_title"] = (
                "Network simulation of " + vis_var
            )
    ...
    def add_rule(self, status, function, rule, schemes=[""]):
        ...
        """
        self.compartment[self.compartment_progressive] = (
            status,
            function,
            rule,
            schemes,
    ...
    def set_initial_status(self, initial_status_funs, configuration=None):
        ...
        if not isinstance(initial_status_funs, dict):
            raise ValueError(
                "The initial status should be a dictionary of form status (str): value (int/float/function)"
            )
            ...
            for status_fun in initial_status_funs.items():
                if hasattr(status_fun[1], "__call__"):
                    status[status_fun[0]] = status_fun[1](
                        node, self.graph, status, self.constants
                    )
                    continue
                if not isinstance(status_fun[1], float):
                    if not isinstance(status_fun[1], int):
                        raise ValueError(
                            "The initial status should be a function, integer, or float"
                        )
                status[status_fun[0]] = status_fun[1]
    ...
    def iteration_bunch(self, bunch_size, node_status=True, progress_bar=False):
        ...
        """
        iterations = super().iteration_bunch(
            bunch_size, node_status, progress_bar=progress_bar
        )
    ...
    def get_mean_data(self, iterations, mean_type):
        ...
        del mean_changes[
            "Infected"
        ]  # fix this line so that infected isn't even in available statuses
    ...
    def build_trends(self, iterations):
        ...
        return {
            "mean_delta_status_vals": means_status_delta_vals,
    ...
    def create_frames(self, iterations):
        ...
        for i in range(len(self.full_status)):
            if i % self.visualization_configuration["plot_interval"] == 0:
                ...
                node_colors.append(
                    [
                        self.full_status[i]["status"][node][
    ...
    def visualize(self, iterations):
        ...
        if not self.visualization_configuration:
            raise Exception(
                "Specify a visualization configuration before you visualize the model"
            )
        ...
        vmax = self.visualization_configuration["variable_limits"][
            self.visualization_configuration["plot_variable"]
        ][1]
        ...
        def updateData(curr):
            ...
            for i, ax in enumerate(axis):
                _, bins, patches = ax.hist(
                    histo_frames[statuses[i]][curr],
                    range=self.visualization_configuration["variable_limits"][
                        statuses[i]
                    ],
                    density=1,
                    bins=25,
            ...
            network.set_title(
                "Iteration: "
                + str(curr * self.visualization_configuration["plot_interval"])
            )
        ...
        simulation = animation.FuncAnimation(
            ...
            updateData,
            n,
            interval=self.visualization_configuration["animation_interval"],
            repeat=True,
        )
    ...
    def save_plot(self, simulation):
        ...
        print(
            "Saving plot at: "
            + self.visualization_configuration["plot_output"]
            + " ..."
        )
        split = self.visualization_configuration["plot_output"].split("/")
        file_name = split[-1]
        file_path = self.visualization_configuration["plot_output"].replace(
            file_name, ""
        )
        if not os.path.exists(file_path):
            os.makedirs(file_path)
        ...
        simulation.save(
            self.visualization_configuration["plot_output"], writer=writergif
        )
