...
class SEISctModel(DiffusionModel):
    ...
    def __init__(self, graph, seed=None):
        ...
        self.available_statuses = {"Susceptible": 0, "Exposed": 2, "Infected": 1}
    ...
    def iteration(self, node_status=True):
        ...
        for u in self.graph.nodes:
            ...
            if u_status == 0:  # Susceptible
                ...
                if eventp < 1 - np.exp(
                    -(self.actual_iteration - self.progress[u])
                    * self.params["model"]["alpha"]
                ):
                ...
                if eventp < 1 - np.exp(
                    -(self.actual_iteration - self.progress_I[u])
                    * self.params["model"]["lambda"]
                ):
