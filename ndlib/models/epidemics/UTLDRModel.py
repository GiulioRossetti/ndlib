...
class UTLDRModel(DiffusionModel):
    ...
    def iteration(self, node_status=True):
        ...
        for u in self.graph.nodes:
            ...
            if (
                self.params["nodes"]["filtered"][u] is not None
                and len(self.params["nodes"]["filtered"][u]) > 0
            ):
                neighbors = list(
                    set(neighbors) & set(self.params["nodes"]["filtered"][u])
                )

            ####################### Undetected Compartment ###########################

            if u_status == self.available_statuses["Susceptible"]:
                actual_status[u] = self.__Susceptible_to_Exposed(
                    u, neighbors, lockdown=False
                )

            elif u_status == self.available_statuses["Exposed"]:

                tested = np.random.random_sample()  # selection for testing
                if not self.params["nodes"]["tested"][
                    u
                ] and tested < self.__get_threshold(u, "phi_e"):
                    res = (
                ...
                if not self.params["nodes"]["tested"][
                    u
                ] and tested < self.__get_threshold(u, "phi_i"):
                    res = (
                        np.random.random_sample()
                    )  # probability of false negative result
                    if res > self.__get_threshold(u, "kappa_i"):
                        ...
                        icup = (
                            np.random.random_sample()
                        )  # probability of severe case needing ICU
    ...
    def __interaction_selection(neighbors, prob):
        return np.random.choice(
            a=neighbors, size=int(len(neighbors) * prob), replace=True
        )
    ...
    def __limit_social_contacts(self, u, neighbors=None, event="Tested"):
        ...
        if (
            self.params["nodes"]["filtered"][u] is None
            or len(self.params["nodes"]["filtered"][u]) > 0
        ):
            return
        ...
        if event == "Tested":
            ...
            remaining_candidates = [
                n
                for n in neighbors
                if self.params["nodes"]["filtered"][n] is not None
                and len(self.params["nodes"]["filtered"][n]) == 0
            ]
            size = min(
                int(len(neighbors) * (1 - filtering_prob)), len(remaining_candidates)
            )
        else:
        ...
        if event != "Tested":
            for n in to_keep:
                ...
                if self.params["nodes"]["filtered"][n] is not None and self.status[
                    n
                ] not in [
                    ...
                    self.params["nodes"]["filtered"][n] = list(
                        (set(to_keep) - {n}) | {u}
                    )
        else:
            # Quarantine
            for n in set(neighbors) - set(to_keep):
                if (
                    self.params["nodes"]["filtered"][n] is not None
                    and len(self.params["nodes"]["filtered"][n]) > 0
                ):
                    # removing the quarantined node from the neighbors' contacts
                    self.params["nodes"]["filtered"][n] = list(
                        set(self.params["nodes"]["filtered"][n]) - {u}
                    )
                else:
                    if self.status[n] not in [
                        self.available_statuses["Identified_Exposed"],
                        self.available_statuses["Hospitalized_mild"],
                        self.available_statuses["Hospitalized_severe_ICU"],
                        self.available_statuses["Hospitalized_severe"],
    ...
    def __Susceptible_to_Exposed(self, u, neighbors, lockdown=False):
        ...
        # vaccination test
        if (
            self.__get_threshold(u, "v") > 0
            and not self.params["nodes"]["vaccinated"][u]
        ):
        ...
        interactions = list(
            self.__interaction_selection(neighbors, self.params["nodes"]["activity"][u])
        )
        ...
        for v in interactions:
            if (
                self.status[v] == self.available_statuses["Infected"]
                or self.status[v] == self.available_statuses["Lockdown_Infected"]
                or self.status[v] == self.available_statuses["Hospitalized_mild"]
            ):
                ...
                if zp < self.__get_threshold(
                    u, "z"
                ):  # infection risk due to partial corpse disposal
                    if lockdown:
                        return self.available_statuses["Lockdown_Exposed"]
                    return self.available_statuses["Exposed"]
