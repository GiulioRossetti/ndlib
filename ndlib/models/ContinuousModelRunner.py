...
class ContinuousModelRunner(object):
    ...
    def run(self, N, iterations_list, initial_statuses, constants_list=None):
        ...
        for i in range(N):
            ...
            self.model.set_initial_status(
                initial_statuses[i % n_initial_statuses], self.config
            )
            output = self.model.iteration_bunch(
                iterations_list[i % n_iterations], node_status=self.node_status
            )
    ...
    def analyze_sensitivity(
        self, sa_type, initial_status, bounds, n, iterations, second_order=True
    ):
        """
        ...
        for i in range(len(param_values)):
            ...
            outputs.append(
                self.model.iteration_bunch(
                    iterations, node_status=self.node_status, progress_bar=False
                )
            )
