    ...
    def iteration(self, node_status=True):
        ...
        if self.actual_iteration == 0:
            ...
            for node in self.graph.nodes:
                if self.params["model"]["similarity"] == 1:
                    ...
                    if len(self.params["nodes"]["vector"][node]) == 0:
                        ...
                        while i < 5:
                            self.params["nodes"]["vector"][node].append(
                                np.random.randint(2)
                            )
                            i += 1
            ...
            if not use_stubborn_node:
                ...
                if (
                    self.params["model"]["option_for_stubbornness"] == -1
                    and len(negatives) != 0
                ):
                    num_stubborns = int(
                        float(len(negatives))
                        * float(self.params["model"]["perc_stubborness"])
                    )
                elif (
                    self.params["model"]["option_for_stubbornness"] == 1
                    and len(positives) != 0
                    ...
                    self.params["model"]["option_for_stubbornness"] == 0
                    and len(join_list) != 0
                ):
                    num_stubborns = int(
                        float(len(join_list))
        ...
        for _ in range(0, self.graph.number_of_nodes()):
            ...
            if self.params["nodes"]["stubborn"][n1] == 0:
                ...
                if self.params["model"]["similarity"] == 1:
                    sum_op = actual_status[n1] + (
                        (actual_status[neigh] * weight) * jaccard_sim
                    )
                else:
                ...
                if self.params["model"]["method_variant"] == 0:
                    ...
                    if diff_opinion <= self.params["model"]["epsilon"]:
                        new_op = self.attraction(
                            actual_status[n1], actual_status[neigh], sum_op
                        )

                elif self.params["model"]["method_variant"] == 2:
                    # interaction with repulsion
                    if diff_opinion > self.params["model"]["epsilon"]:
                        new_op = self.repulsion(
                            actual_status[n1], actual_status[neigh], sum_op
                        )
