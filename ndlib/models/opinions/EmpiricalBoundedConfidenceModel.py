...
class EmpiricalBoundedConfidenceModel(DiffusionModel):
    ...
    def set_initial_status(self, configuration=None, initialstatus=None, ):
        """
    ...
    def iteration(self, node_status=True):
        ...
        for i in range(n):
            ...
            n2 = int(rng.choice(neigh_ids, 1, p=[self.actrates[i]/sum(self.actrates[neigh_ids]) for i in neigh_ids]))
    ...
    def steady_state(self, path, run, max_iterations=100000, nsteady=1000, sensibility=0.00001, node_status=True,
                     progress_bar=False):
        """
        ...
        for it in tqdm.tqdm(range(0, max_iterations), disable=not progress_bar):
            ...
            with open(f'{path}/edgelist_n{n}_maxit{max_it}_run{run}.csv', 'a+') as ofile:
                for edge in ints:
                    s = f'{it},'
                    for node in edge:
                        s+=f'{node},'
