...
class CognitiveOpDynModel(DiffusionModel):
	...
	for node in self.graph.nodes():
		...
		self.params['nodes']['cognitive'][node]=(RDomain[np.random.randint(0,3)],np.random.random_sample(),np.random.random_sample())
	...
	for node in self.graph.nodes():
		...
		if R==-1:
			self.status[node]=0.5* self.status[node]
	...
	for i in range(0,self.graph.number_of_nodes()):
		...
		if np.random.random_sample()<p2:#if node 2 talks, node 1 gets changed
			...
			R1=self.params['nodes']['cognitive'][n1][0]
            		self.status[n1] = self.status[n1]+(1-T1)*(self.status[n2]-self.status[n1])
			if R1==1:
				self.status[n1]=0.5*(1-self.status[n1])
			if R1==-1:
				self.status[n1]=0.5* self.status[n1]
		...
		if np.random.random_sample()<p1:#if node 1 talks, node 2 gets changed
			...
			R2=self.params['nodes']['cognitive'][n2][0]
            		self.status[n2] = self.status[n2]+(1-T2)*(oldn1-self.status[n2])
			if R2==1:
				self.status[n2]=0.5*(1-self.status[n2])
			if R2==-1:
				self.status[n2]=0.5* self.status[n2]
